...
export const Checkboxes = <T extends ICheckboxValue>(props: CheckboxesProps<T>) => {
  ...
  return (
    <CheckboxesWrapper className={props.className} $width={props.width} $size={props.size}>
      {props.options.map((e, index) => {
        const isChecked = props.value.some(selectedItem => selectedItem[props.valueField] === e[props.valueField])
        return (
          <CheckboxesItem key={index} onClick={() => !props.isClickOnlyIcon && handleCheckboxChange(e)}>
            <Checkbox
            ...
            >
              {props.labelField && e?.[props.labelField] !== undefined && (
                <CheckboxesLabel dangerouslySetInnerHTML={createMarkup(e[props.labelField] as string)} />
              )}
            </Checkbox>
            {props.childrenField && e?.[props.childrenField] !== undefined && (
              <CheckboxChildren>{e[props.childrenField] as React.ReactNode}</CheckboxChildren>
            )}
          </CheckboxesItem>
        )
      })}
